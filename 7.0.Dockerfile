...
# NOTE: We use 8.0 requirements.txt, because 7.0 didn't have one
RUN debs="python-dev build-essential libxml2-dev libxslt1-dev libjpeg-dev libfreetype6-dev liblcms2-dev libopenjpeg-dev libtiff5-dev tk-dev tcl-dev linux-headers-amd64 libpq-dev libldap2-dev libsasl2-dev" \
    ...
    && pip install \
        -r https://raw.githubusercontent.com/$ODOO_SOURCE/8.0/requirements.txt \
        phonenumbers \
        watchdog \
...
LABEL org.label-schema.schema-version="$VERSION" \
      org.label-schema.vendor=Pragmatic \
      org.label-schema.license=Apache-2.0 \
      org.label-schema.build-date="$BUILD_DATE" \
      org.label-schema.vcs-ref="$VCS_REF" \
      org.label-schema.vcs-url="https://github.com/pragmatic-lab/docker-odoo"
...
RUN pip install --user --no-cache-dir --no-deps $SOURCES/odoo
